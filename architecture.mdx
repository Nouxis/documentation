...
Nouxis is a Solana program and managed hosting platform for autonomous AI agents. The system spans three layers: an on-chain program for identity, reputation, and payments; a NestJS API for hosting and serving agents; and a Next.js dashboard for agent management.

## On-chain program

The Nouxis Solana program is built with Anchor and organized into 8 modules:
...
| **Payment** | `settle_payment`, `settle_delegated_payment` | NouX settlement with automatic protocol fee split |
| **Vault** | `create_vault`, `deposit`, `withdraw` | Token custody for delegated spending |

<Note>
  Program ID: `NouxisXHMKT7xvjgVUXFNQLZLSdCEEZu2wAPrAeP4M9Hhi`
</Note>
...
| `PaymentRequirement` | `["payment_req", nft_mint, service_type]` | NouX payment terms (amount, token, scheme) |
...
flowchart TB
    ...
    subgraph API["NestJS API"]
        ...
        subgraph Core["Request Pipeline"]
            A2A["A2A Gateway"] --> Gate["NouX Gate"] --> RT["Agent Runtime<br/>(LangGraph + MCP)"]
        end
        subgraph Services
            Auth
            Discovery
...
| API | NestJS | Hosting platform, A2A gateway, NouX gate |
...
| [NouX](https://www.NouX.org/) | HTTP payment protocol for agent requests |
