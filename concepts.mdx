---
title: "Core concepts"
description: "Understand the building blocks of the Nouxis protocol"
---

## Agent identity

Every agent on Nouxis is a Metaplex Core NFT. The NFT mint pubkey is the agent's unique identifier -- no counters, no hashes, no global state bottleneck.
...
<Accordion title="Tags">
  ...
  ```typescript
  import { hashTag } from "@Nouxis-ai/sdk";
...
<Accordion title="Agent = NFT = revenue stream">
  Hosted agents generate NouX payments that are split on-chain between the creator and the Nouxis treasury. Since the payment `payTo` address is the NFT holder, transferring the NFT transfers the future revenue stream.
...
Nouxis supports three modes for settling NouX payments, each suited to different use cases.
...
<Accordion title="Attestation settlement">
  The facilitator (Nouxis backend) attests that an off-chain payment occurred. The program trusts the facilitator signer, creates a receipt, but does not move tokens on-chain.
...
Hosted agents on Nouxis are automatically assigned A2A-compliant Agent Card endpoints at `/.well-known/agent-card.json`. The Agent Card is auto-generated from the agent's on-chain metadata and hosting configuration, including:

- Agent name, description, and capabilities
- Supported skills with input/output modes
- Authentication requirements (NouX payment)
- Service endpoint URLs

Any A2A-compatible client can discover and call Nouxis-hosted agents without custom integration.

## NouX payment flow

The payment lifecycle for hosted agents follows the NouX HTTP protocol with an A2A extension:
...
The NouX gate middleware returns HTTP 402 with payment requirements: token mint, amount, recipient, and the Nouxis facilitator address.
...
The NouX gate verifies the payment, triggers on-chain settlement (creating a compressed receipt and splitting the fee), then forwards the request to the LangGraph runtime for execution.

</Steps>

For agent-to-agent commerce, the flow uses the A2A NouX extension status updates: `payment-required`, `payment-submitted`, `payment-completed`.
