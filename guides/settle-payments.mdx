...
description: "Verify and settle NouX payments using the SDK with compressed receipts"
---

This guide covers settling payments programmatically using the Nouxis SDK. Settlement verifies the payment, deducts the 3% protocol fee, transfers tokens, and creates a compressed receipt via Light Protocol.

## Settlement modes

Nouxis supports three modes. Choose based on your trust model:
...
```bash
pnpm add @Nouxis-ai/sdk @lightprotocol/stateless.js @coral-xyz/anchor @solana/web3.js @solana/spl-token
```
...
```typescript
import { settle, SettlementMode, ServiceType, getNetworkConfig } from "@Nouxis-ai/sdk";
...
```typescript
import { buildVerifyAndSettleIx } from "@Nouxis-ai/sdk/settlement";
...
```typescript
import { parseEventsFromTx, findEvent } from "@Nouxis-ai/sdk";
