...
description: "Create a pre-authorized budget for zero-signature NouX payments"
---

Budgets let agents or applications make NouX payments without signing each transaction. You approve a spending limit once, and the Nouxis facilitator auto-debits within those constraints. This is essential for agent-to-agent commerce and automated workflows.

## How it works

A budget combines two Solana primitives in a single transaction:

1. **SPL Token approve** -- gives the Nouxis facilitator transfer authority over your token account
2. **Nouxis Delegation PDA** -- enforces constraints: per-tx limit, total budget, allowed tokens, expiry, use count
...
- The Nouxis facilitator address (from `getNetworkConfig`)

## Install dependencies

```bash
pnpm add @Nouxis-ai/sdk @coral-xyz/anchor @solana/web3.js @solana/spl-token
```
...
<Steps>
  <Step title="Initialize the client">
    ...
    import { DelegationBuilder, getNetworkConfig } from "@Nouxis-ai/sdk";
  ...
  <Step title="SPL Token approve">
    Authorize the Nouxis facilitator as a delegate on your token account.

    ```typescript
    const approveIx = createApproveInstruction(
      payerTokenAccount,          // your USDC ATA
      config.facilitator!,        // Nouxis facilitator wallet
...
```bash
curl -X POST https://a2a.Nouxis.ai/agents/{nft-mint}/v1 \
...
```typescript
import { fetchDelegation } from "@Nouxis-ai/sdk";
