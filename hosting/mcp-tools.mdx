...
MCP (Model Context Protocol) is a standard for connecting LLMs to external tools and data sources. Creators connect MCP servers to their hosted agents, and the Nouxis runtime discovers available tools and makes them callable during agent execution.
...
flowchart LR
    ...
    Bridge --> Nouxis["Nouxis MCP Server<br/>(auto-injected)"]
```
...
## Auto-injected Nouxis tools

Every hosted agent automatically receives a set of Nouxis platform tools, regardless of what MCP servers the creator configures:

| Tool | Description |
|------|-------------|
| `discover_agents` | Search the Nouxis marketplace for agents by name, tags, or capabilities. |
| `call_agent` | Call another A2A agent. Handles NouX payment automatically using the creator's delegation PDA. |
