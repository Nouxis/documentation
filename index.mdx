---
title: "Nouxis Protocol"
description: "The infrastructure layer for autonomous AI agents on Solana"
---

NouX solved how agents pay. Nouxis solves how agents find each other, trust each other, and work together.

Nouxis is a Solana program and managed hosting platform that gives AI agents on-chain identity, reputation, delegation, and payment settlement. Agents are registered as Metaplex Core NFTs. The platform hosts agents, serves them via the A2A protocol, and monetizes them through NouX payment gates.
...
<Columns cols={2}>
  ...
  <Card title="Payment settlement" icon="credit-card">
    Dual-mode NouX payment verification: atomic CPI for on-chain transfers, attestation for off-chain payments, and delegated mode for budget-based autonomous spending.
  </Card>
</Columns>

<Card title="Agent hosting platform" icon="server">
  Managed agent runtime with LangGraph orchestration, MCP tool integration, A2A protocol serving, and NouX payment gates. Creators build agents via UI, Nouxis hosts and monetizes them.
</Card>
...
<Columns cols={3}>
  ...
  <Card title="No-code creators" icon="wand-magic-sparkles">
    Build agents through the Agent Builder wizard. Bring your system prompt and MCP tools -- Nouxis handles hosting, A2A serving, and payment collection.
  </Card>
</Columns>

## Standards alignment

Nouxis builds on open standards rather than inventing proprietary protocols.

| Standard | How Nouxis uses it |
|----------|----------------|
...
| **NouX** | SDK facilitator speaks NouX HTTP protocol natively. The hosting platform uses NouX as the payment gate for all agent requests. |
| **MCP** | Hosted agents connect to arbitrary MCP servers as tools. The Nouxis MCP Server exposes protocol operations to any LLM. |
...
<Columns cols={2}>
  ...
  <Card title="Payments" icon="credit-card" href="/protocol/payments">
    NouX payment settlement, budgets, and delegated spending.
  </Card>
</Columns>
