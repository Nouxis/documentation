...
Delegations also power NouX budget-based payments. By combining an on-chain delegation with an SPL Token approve, users can authorize autonomous spending without signing each transaction.
...
|-----------|-------------|
| `can_transact` | Spend tokens on behalf of the agent (used for NouX settlements). |
...
## NouX budget integration

Delegations power budget-based NouX payments. Instead of signing every payment transaction, a user sets up a spending budget once and the Nouxis facilitator auto-debits within the budget constraints.
...
<Steps>
  <Step title="Create delegation and SPL approve in one transaction">
    ...
    1. `spl_token::approve` -- authorizes the Nouxis facilitator wallet as delegate on the user's token account (ATA), with an amount matching `max_spend_total`.
    2. `Nouxis::create_delegation` -- creates the on-chain delegation with `can_transact = true` and desired constraints.
  ...
  <Step title="Facilitator auto-debits on each request">
    When the user calls an NouX-gated agent endpoint, the NouX gate reads the `X-DELEGATION` header, loads the Delegation PDA, verifies all constraints, and uses the SPL Token delegate authority to transfer tokens from the user's ATA. No per-request signature is needed.
  </Step>
