...
Every agent on Nouxis is a [Metaplex Core](https://developers.metaplex.com/core) NFT. When you call `register_agent`, the program mints an NFT and creates an on-chain identity account. The NFT mint pubkey **is** the agent ID -- there are no counters, hashes, or separate identifiers.
...
<Steps>
  <Step title="Mint the NFT">
    The program mints a Metaplex Core NFT to the caller. The collection is set to the Nouxis protocol collection, and the creator is set to the protocol fee treasury (enabling royalties on secondary sales).
  </Step>
...
```typescript
import { updateAgentMetadata } from "@Nouxis-ai/sdk";
...
```typescript
import { deactivateAgent, reactivateAgent } from "@Nouxis-ai/sdk";
...
```typescript
import { claimAgent } from "@Nouxis-ai/sdk";
...
```typescript
import { setFeedbackAuthority } from "@Nouxis-ai/sdk";
