...
## How Nouxis implements NouX

Each agent registers a `PaymentRequirement` PDA that defines pricing for a specific service type. When a request hits the NouX gate:
...
sequenceDiagram
    participant Client
    participant NouX Gate
    participant Solana
    participant Agent

    Client->>NouX Gate: POST /v1/chat
    NouX Gate-->>Client: 402 + PaymentRequirement
    Client->>Client: Sign SPL transfer
    Client->>NouX Gate: POST /v1/chat + X-PAYMENT header
    NouX Gate->>Solana: verify_and_settle
    Solana-->>NouX Gate: Receipt created
    NouX Gate->>Agent: Forward request
    Agent-->>Client: Response
```

## Settlement modes

Nouxis supports three settlement modes, each suited to different trust and automation levels.
...
Every settlement creates a payment receipt. At scale, receipts are the highest-volume account type in the protocol. Nouxis uses [Light Protocol](https://zkcompression.com) (ZK Compression) to store receipts as compressed PDAs instead of regular Solana accounts, eliminating rent costs entirely.
...
```bash
pnpm Nouxis -n devnet -a create-alt
```
...
A budget is an on-chain spending allowance that lets agents or applications make NouX payments without requiring a human signature per request. It combines two Solana primitives:

1. **SPL Token approve** -- gives the Nouxis facilitator wallet transfer authority over the payer's token account
2. **Nouxis Delegation PDA** -- enforces granular constraints (per-tx limit, total budget, allowed tokens, expiry, use count)

Together, these allow the NouX gate to auto-debit payments within the budget's constraints. The payer signs once to set up the budget, then all subsequent payments within the limits happen autonomously.
...
<Steps>
  <Step title="SPL Token approve">
    Authorize the Nouxis facilitator wallet as a delegate on the payer's token account. The approved amount should match the budget's `max_spend_total`.
    ...
    const approveIx = createApproveInstruction(
      ...
      facilitatorPubkey,     // Nouxis facilitator wallet
  ...
  <Step title="Create delegation">
    ...
    ```typescript
    import { DelegationBuilder } from "@Nouxis-ai/sdk";
...
### Three payment paths in the NouX gate

When a request reaches a paid agent endpoint, the NouX gate checks for payment authorization in this order:

| Header | Mode | Description |
|--------|------|-------------|
| `X-PAYMENT` | Atomic or Attestation | Standard NouX flow. Client provides signed payment proof. |
...
```typescript
import { revokeDelegation } from "@Nouxis-ai/sdk";
import { createRevokeInstruction } from "@solana/spl-token";

// Revoke the Nouxis delegation
...
<Accordion title="Marketplace subscriptions">
  A consumer creates a budget allowing an agent to charge up to 50 USDC/month for ongoing access. The agent's NouX gate auto-debits each request against the budget. When the budget runs out or expires, the consumer tops it up or creates a new one.
</Accordion>
...
<Accordion title="SDK programmatic access">
  An application uses a hot wallet to interact with paid agents. The application creates a budget from the hot wallet to the Nouxis facilitator, then includes the delegation address in all API calls. No human in the loop for each request.
</Accordion>
...
import { settle, SettlementMode } from "@Nouxis-ai/sdk/settlement";
