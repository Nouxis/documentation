...
```bash npm
npm install @Nouxis-ai/sdk @coral-xyz/anchor @solana/web3.js @solana/spl-token
```

```bash yarn
yarn add @Nouxis-ai/sdk @coral-xyz/anchor @solana/web3.js @solana/spl-token
```
</CodeGroup>
...
<Steps>
  <Step title="Set up connection and program">
    Connect to Solana devnet and initialize the Nouxis program client.
  ...
  <Step title="Add a service endpoint">
    ...
    ```typescript
    import { addService } from "@Nouxis-ai/sdk";
    ...
    Nouxis supports four service types: `A2A`, `MCP`, `API`, and `Web`.
  </Step>
  <Step title="Set a payment requirement">
    Define an NouX payment gate for your service. This tells callers how much to pay and which token to use.

    ```typescript
    import { setPaymentRequirement, PaymentScheme } from "@Nouxis-ai/sdk";
...
<Columns cols={2}>
  ...
  <Card title="Agent hosting" icon="server" href="/hosting/overview">
    Host your agent on Nouxis with A2A serving and NouX payment gates.
  </Card>
</Columns>
