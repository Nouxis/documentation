---
title: "SDK overview"
description: "TypeScript SDK for the Nouxis Solana protocol"
---

The `@Nouxis-ai/sdk` package provides TypeScript bindings for every instruction, account type, and utility in the Nouxis Solana program. It handles PDA derivation, instruction building, account fetching, event parsing, and error handling.

## Installation

<CodeGroup>
```bash pnpm
pnpm add @Nouxis-ai/sdk
# Peer dependencies
pnpm add @coral-xyz/anchor @solana/web3.js @solana/spl-token
```

```bash npm
npm install @Nouxis-ai/sdk
# Peer dependencies
npm install @coral-xyz/anchor @solana/web3.js @solana/spl-token
```

```bash yarn
yarn add @Nouxis-ai/sdk
# Peer dependencies
...
| Import | Purpose |
|--------|---------|
| `@Nouxis-ai/sdk` | Core SDK -- instructions, types, PDA helpers, account fetchers, events, errors |
| `@Nouxis-ai/sdk/settlement` | Settlement -- Light Protocol integration, compressed receipts, ALT management |

```typescript
import { ... } from "@Nouxis-ai/sdk";              // Core SDK
import { ... } from "@Nouxis-ai/sdk/settlement";    // Settlement (Light Protocol)
```
...
```typescript
import {
  Nouxis_PROGRAM_ID,        // Program ID: NouxisXHMKT7xvjgVUXFNQLZLSdCEEZu2wAPrAeP4M9Hhi
...
| Field | Description |
|-------|-------------|
| `programId` | Nouxis program address |
...
All PDA functions return `[PublicKey, number]`. The default `programId` is `Nouxis_PROGRAM_ID`.
...
```typescript
import { parseNouxisError, NouxisError, NouxisErrorCode } from "@Nouxis-ai/sdk";

try {
  await program.methods.registerAgent(...).rpc();
} catch (err) {
  const Nouxiserr = parseNouxisError(err);
  if (Nouxiserr) {
    console.log(Nouxiserr.code, Nouxiserr.message);
