...
The settlement module handles NouX payment verification and execution, including Light Protocol compressed receipts for cost-efficient on-chain storage at scale.
...
```typescript
import { settle, SettlementMode, ServiceType } from "@Nouxis-ai/sdk/settlement";
...
```typescript
import { prepareLightSettlement } from "@Nouxis-ai/sdk/settlement";
...
```typescript
import { deriveCompressedReceiptAddress } from "@Nouxis-ai/sdk/settlement";
...
pnpm Nouxis -n devnet -a initialize

# Update on-chain config (e.g., change fee, set facilitator)
pnpm Nouxis -n devnet -a update-config

# Deploy or upgrade the program
pnpm Nouxis -n devnet -a deploy-program

# Generate a named keypair
pnpm Nouxis -n devnet -a generate-keypair
```
